...
const MY_STOCKS = [
  ...
  async function fetchStockData(code) {
    ...
    console.log(`[${new Date().toLocaleString()}] ${code}: 名称=${name}, 当前价=${price}, 昨收=${preClose}, 涨跌幅=${((price - preClose) / preClose * 100).toFixed(2)}%`)
    
    return {
      name: name,
      price: price,
      pre_close: preClose,
      change_percent: ((price - preClose) / preClose * 100).toFixed(2)
    }
  }
